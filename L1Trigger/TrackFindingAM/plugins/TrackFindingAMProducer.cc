...
void TrackFindingAMProducer::produce( edm::Event& iEvent, const edm::EventSetup& iSetup )
{
  ...
  if ( TTStubHandle->size() > 0 )
  {
    ...
    for (auto gd=theTrackerGeom->dets().begin(); gd != theTrackerGeom->dets().end(); gd++) 
    {
      ...
      //      std::cout << "AMAA " << static_cast<int>(detid) << std::endl;
      ...
      //  std::cout << "AMA " << static_cast<int>(detid) << std::endl; 
	...
	{	
	  ...
	  //	  if (static_cast<int>(tTopo->tidWheel(detid))>=3 && m_tilted) ladder += 2;

	  module = static_cast<int>(tTopo->module(detid))-1;
	  type   = 0;
	}

	//	std::cout << "AM " << static_cast<int>(detid) << std::endl; 
