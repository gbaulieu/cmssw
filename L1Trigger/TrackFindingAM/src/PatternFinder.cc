...
void PatternFinder::find(int start, int& stop){
  ...
  if(outputFileName.compare(eventsFileName)==0){//no output file given -> we will write output in input file
    rootFile->Delete(tree_name_delete.c_str());    
  }
  else{//output file given -> we store results in this file
    rootFile_output = new TFile(outputFileName.c_str(),"update");
    rootFile_output->Delete(tree_name_delete.c_str());    
  }
