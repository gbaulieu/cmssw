...
INDIR_GRID=$CMSSW_BASE/src/L1Trigger/TrackFindingAM/test/input
INDIR_XROOT=$INDIR_GRID

# The SE directory containing the output EDM file with the PR output
OUTDIR_GRID=$CMSSW_BASE/src/L1Trigger/TrackFindingAM/test/input
OUTDIR_XROOT=$OUTDIR_GRID
...
for ll in `ls $INDIR_GRID | grep root` 
do   
    l=`basename $ll`
    echo $l
    i=0
    ...
    dealF=`ls $AMPR_FILE | wc -l`
    dealE=`ls $FIT_FILE | wc -l`
    dealD=`ls $EXTR_FILE | wc -l`
	...
	echo "$PACKDIR/PR_processor_parallel.sh PRMERGE all.root $INDIR_XROOT $l ${OUTM}_ $RELEASEDIR $GTAG ${OUTM}_all $OUTDIRSTO $BANKDIR 0 -1 $OUTDIRTMP $GEOM" >> run_PRMERGE_${nsj}_${MATTER}.sh
	...
	do
	    ...
	    echo "$PACKDIR/PR_processor_parallel.sh PRMERGE ${i}_${j}.root $INDIR_XROOT $l ${OUTM}_ $RELEASEDIR $GTAG ${OUTM}_${i}_${j} $OUTDIRSTO $BANKDIR $i $NPFILE $OUTDIRTMP $GEOM" >> run_PRMERGE_${nsj}_${MATTER}.sh
    ...
    echo "$PACKDIR/PR_processor_parallel.sh  FINAL MERGED_${OUTM}_ $OUTDIRTMP $OUTDIRTMP $OUTF $RELEASEDIR ${OUTM} $OUTDIRSTO $GEOM"  >> run_FMERGE_${nsj}_${MATTER}.sh
    echo "$PACKDIR/PR_processor_parallel.sh  FIT $OUTDIRTMP/${OUTF} $OUTE $OUTD $NTOT $OUTDIR_GRID $RELEASEDIR $GTAG ${OUTM} $OUTDIRSTO $GEOM" >> run_FIT_${nsj}_${MATTER}.sh
