...
do   
	...
	if [ $NCORES = 1 ]; then
	    echo "$PACKDIR/batch/run_PRMERGE_${nsj}_${MATTER}.sh" >> run_${nsj}_${MATTER}.sh
	    ...
	    echo "${parallel} -j ${NCORES} < $PACKDIR/batch/run_PRMERGE_${nsj}_${MATTER}.sh" >> run_${nsj}_${MATTER}.sh
	...
	echo "$PACKDIR/batch/PR_processor_parallel_p.sh PRMERGE ${i}_${j}.root $INDIR_XROOT $l ${OUTM}_ $RELEASEDIR $GTAG ${OUTM}_${i}_${j} $OUTDIRSTO $BANKDIR $i $NPFILE $OUTDIRTMP" >> run_PRMERGE_${nsj}_${MATTER}.sh
    ...
    echo "$PACKDIR/batch/PR_processor_parallel_p.sh  FINAL MERGED_${OUTM}_ $OUTDIRTMP $OUTDIRTMP $OUTF $RELEASEDIR ${OUTM} $OUTDIRSTO  $OUTDIR_GRID"  >> run_FMERGE_${nsj}_${MATTER}.sh
    echo "$PACKDIR/batch/PR_processor_parallel.sh  FIT $OUTDIRTMP/${OUTF} $OUTE $OUTD $NTOT $OUTDIR_GRID $RELEASEDIR $GTAG ${OUTM} $OUTDIRSTO" >> run_FIT_${nsj}_${MATTER}.sh
