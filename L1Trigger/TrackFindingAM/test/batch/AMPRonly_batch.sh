...
INDIR_GRID=$SRMPATH/GEN/$INPUTDIR
INDIR_XROOT=$XRDPATH/GEN/$INPUTDIR

# The SE directory containing the output EDM file with the PR output
OUTDIR_GRID=$SRMPATH/PR/910/${INPUTDIR}_withAMPR
OUTDIR_XROOT=$XRDPATH/PR/910/${INPUTDIR}_withAMPR
...
for ll in `gfal-ls $INDIR_GRID | grep EDM` 
do   
    l=`basename $ll`
    echo $l
    i=0
...
#    if [ $dealF != "0" ] && [ $dealE != "0" ] && [ $dealD != "0" ]; then
	...
	echo "$PACKDIR/PR_processor_parallel.sh PRMERGE all.root $INDIR_GRID/$l $l ${OUTM}_ $RELEASEDIR $GTAG ${OUTM}_all $OUTDIRSTO $BANKDIR 0 -1 $OUTDIRTMP" >> run_PRMERGE_${nsj}_${MATTER}.sh
	...
	do
	    ...
	    echo "$PACKDIR/PR_processor_parallel.sh PRMERGE ${i}_${j}.root $INDIR_GRID/$l $l ${OUTM}_ $RELEASEDIR $GTAG ${OUTM}_${i}_${j} $OUTDIRSTO $BANKDIR $i $NPFILE $OUTDIRTMP" >> run_PRMERGE_${nsj}_${MATTER}.sh
