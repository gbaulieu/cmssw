...
if [ ${1} = "PRMERGE" ]; then
    ...
    echo "Calling PRMERGE with following options :"
    echo "Input file: "$INPUTDIR/$INPUTFILE
...
#    cp $INPUTDIR/$INPUTFILE .
    ...
    do
       ...
       cp $CMSSW_PROJECT_SRC/src/L1Trigger/TrackFindingAM/test/batch/base/AMPR_base.py BH_dummy_${OUTS1}_${SECNUM}.py
...
if [ ${1} = "FINAL" ]; then
    ...
    lcg-cp file://$TOP/$OUTPUTFILE      ${OUTDIR}/$OUTPUTILE

    deal=`lcg-ls ${OUTDIR}/$OUTPUTFILE | wc -l`

    if [ $deal = "0" ]; then
	mv $TOP/$OUTPUTFILE ${INTMP}/RECOVERY/$OUTPUTFILE
    fi
fi
